    ...
    control = list(), intercept = TRUE, Amat=NULL, bvec=bvec, meq=0) 
{
    ...
    if (EMPTY) {
        ...
        for (iter in 1L:control$maxit) {
            ...
            fit.org <- lm.fit(x=x[good, , drop = FALSE]*w, y=z*w, singular.ok=FALSE, 
                            tol=min(1e-07, control$epsilon/1000))
            
            if(!is.null(Amat)) {
              Z <- x[good,,drop = FALSE]*w
              Y <- z*w
              XX <- t(Z) %*% Z
              Xy <- t(Z) %*% Y
              fit <- quadprog:::solve.QP(Dmat = XX, dvec = Xy, Amat = t(Amat), 
                                         bvec = bvec, meq = meq)
            ...
            if (nobs < fit.org$rank) 
                stop(gettextf("X matrix has rank %d, but only %d observations", 
                  fit.org$rank, nobs), domain = NA)
            start[fit.org$qr$pivot] <- fit$coefficients
            eta <- drop(x %*% start)
        ...
        if (fit.org$rank < nvars) 
            coef[fit.org$qr$pivot][seq.int(fit.org$rank + 1, nvars)] <- NA
        xxnames <- xnames[fit.org$qr$pivot]
        residuals <- (y - mu)/mu.eta(eta)
        fit.org$qr$qr <- as.matrix(fit.org$qr$qr)
        nr <- min(sum(good), nvars)
        if (nr < nvars) {
            Rmat <- diag(nvars)
            Rmat[1L:nr, 1L:nvars] <- fit.org$qr$qr[1L:nr, 1L:nvars]
        }
        else Rmat <- fit.org$qr$qr[1L:nvars, 1L:nvars]
        Rmat <- as.matrix(Rmat)
        Rmat[row(Rmat) > col(Rmat)] <- 0
        names(coef) <- xnames
        colnames(fit.org$qr$qr) <- xxnames
    ...
    names(y) <- ynames
    if (!EMPTY) names(fit.org$effects) <- c(xxnames[seq_len(fit.org$rank)], rep.int("", sum(good) - fit.org$rank))
